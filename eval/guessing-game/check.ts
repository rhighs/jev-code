...
async function play(workspace: string): Promise<CheckResult & { feedback: number }> {
  let lo = cfg.range.min, hi = cfg.range.max, guesses = 0, feedback = 0, correct = false, bad: string | undefined;
  ...
  const res = await runPython(workspace, ['main.py'], {
    ...
    onLine: (line, write) => {
      ...
      if (/too high|lower/.test(text)) { hi = guess - 1; feedback++; }
      else if (/too low|higher/.test(text)) { lo = guess + 1; feedback++; }
      else if (text.trim()) { bad = `unrecognized line: ${line.trim()}`; return; }
      else return;
      next(write);
    },
  });
  if (correct && res.code === 0) return { ok: true, reason: `correct after ${guesses} guesses`, feedback };
  if (bad) return { ok: false, reason: bad, feedback };
  if (res.timedOut) return { ok: false, reason: 'timed out waiting for feedback', feedback };
...
export default async function check(workspace: string): Promise<CheckResult> {
  ...
  for (let round = 0; round < 3; round++) {
    const res = await play(workspace);
    if (!res.ok) return res;
    if (res.feedback > 0) return { ok: true, reason: res.reason };
  }
  return { ok: false, reason: 'program never produced too high/too low feedback' };
}
