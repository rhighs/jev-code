...
if [ -z "$node_bin" ]; then
  case "$(uname -s)" in Darwin) platform=darwin ;; Linux) platform=linux ;; *) fail 'Automatic Node installation supports macOS and Linux only. Install Node.js 22+ with Corepack first.' ;; esac
