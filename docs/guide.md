...
Commands: `provider login [id]` runs the picker (provider, authentication, model; live model lists come from `/models` when the provider has one); `provider logout [id]`; `provider list [--json]`; `provider models [id] [--json]`; `provider use <id|none> [--model <id>] [--base-url <url>]` for scripted setups together with `JEV_GENERATION_API_KEY`. On the first interactive run without a `generation` section, the session asks once whether to configure a provider. Non-interactive runs never ask; without a usable provider, `propose` is not registered and nothing else changes.
