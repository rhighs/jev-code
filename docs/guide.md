...
| `write_files` | Write a multi-module Python project (a package plus `main.py`) as one validated set: every file is staged before any is renamed into place, so a failure writes nothing. |
