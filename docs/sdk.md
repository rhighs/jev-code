...
- `Program` composes immutable decision work with deterministic transforms, dependent continuation, and bounded parallelism.
...
The package requires Node.js 22 or newer and is ESM-only. npm publication is not part of the current release process; from a source checkout, build and pack it before installing the tarball in another project:

```bash
npm ci
npm pack
npm install /path/to/jev-code-0.1.0.tgz
```
...
`router.program(id, instructions, state)` embeds the same selection in a `Program` and uses that run's shared decision resources.

See [`examples/router.ts`](../examples/router.ts) for an executable deterministic example.

## Compose decision programs

Programs are reusable immutable definitions. Every call to `runProgram` gets isolated values and events while sharing one bounded resource scope within that run.

```typescript
import { node, parallel, runProgram, value } from 'jev-code';
...
const program = parallel('deployment-inputs', [region, checks])
  .map('deployment', ([selectedRegion, passed]) => ({ selectedRegion, ready: passed.every(Boolean) }));
...
The grammar is checked before execution for empty IDs, duplicate slot identities, duplicate production IDs, missing children, and cycles. A failed child, node validator, final validator, depth bound, or resource limit never returns a partial tree.
...
Caller cancellation produces `cancelled`; a deadline or consumed limit produces `exhausted` with structured evidence. Validators receive the active signal and resources. Event observers are delivered serially with backpressure. Retained events remain ordered by `sequence`; `metadata.droppedEvents` reports earlier events omitted from the bounded tail.
...
```typescript
import { DecisionSession, defineRouter, runProgram, runTree } from 'jev-code';
```
