...
import { RunResources, type RunLimits, type RunResourceOptions } from './resources.js';
import {
  CancelledError,
  LimitError,
...
export interface ProgramRunOptions<Output> extends RunResourceOptions {
...
type ParallelInput<Programs extends readonly AnyProgram[]> =
  Programs[number] extends Program<infer Input, any> ? Input : never;
...
const validateStructure = (definition: Definition, active = new Set<Definition>()): void => {
  ...
  if (definition.kind === 'parallel') {
    ...
    for (const child of definition.children) validateStructure(child, active);
  } else if (definition.kind === 'map' || definition.kind === 'flatMap' || definition.kind === 'scope') {
    validateStructure(definition.source, active);
  }
...
export class Program<Input, Output> {
  ...
  static value<Input, Output>(id: string, output: Output | ((input: Input) => Output | Promise<Output>)): Program<Input, Output> {
    return Program.node(id, ({ input }) => typeof output === 'function'
      ? (output as (value: Input) => Output | Promise<Output>)(input)
      : output);
  }
...
const emit = async <Input>(runtime: Runtime<Input>, event: ProgramEventInput): Promise<void> => {
  ...
  if (runtime.onEvent !== undefined) {
    const delivered = runtime.eventTail.then(async () => { await runtime.onEvent?.(complete); }).catch(() => {});
...
const contextFor = <Input>(runtime: Runtime<Input>, resources: RunResources, nodeId: string, path: string): ProgramContext<Input> => {
  const context: ProgramContext<Input> = {
    ...
    ...(runtime.decisions === undefined ? {} : {
      decisions: runtime.decisions.withResources(resources).observe(result => {
...
const runNode = async <Input>(
  ...
  try {
    const result = await resources.withPermitScope(async scoped => {
      await emit(runtime, { type: 'node-started', nodeId: id, path, depth });
      return operation(contextFor(runtime, scoped, id, path));
    });
    resources.throwIfAborted();
    await emit(runtime, { type: 'node-completed', nodeId: id, path, depth });
    return result;
  } catch (error) {
    if (runtime.primaryError !== undefined && error === runtime.controller.signal.reason) {
      await emit(runtime, { type: 'node-cancelled', nodeId: id, path, depth, detail: detailOf(error) });
    } else if (error instanceof ProgramInvalidError || error instanceof ProgramDefinitionError) {
      await emit(runtime, { type: 'node-invalid', nodeId: id, path, depth, detail: error.message });
      registerFailure(runtime, error);
    } else if (error instanceof LimitError && (error.evidence.kind === 'exhausted' || error.evidence.kind === 'deadline-exceeded')) {
      await emit(runtime, { type: 'node-exhausted', nodeId: id, path, depth, evidence: error.evidence });
      registerFailure(runtime, error);
    } else if (error instanceof CancelledError) {
      await emit(runtime, { type: 'node-cancelled', nodeId: id, path, depth, detail: error.message });
      registerFailure(runtime, error);
    } else {
      await emit(runtime, { type: 'node-failed', nodeId: id, path, depth, detail: detailOf(error) });
      registerFailure(runtime, error);
    }
    throw error;
  }
...
const evaluate = async <Input>(
  ...
  if (definition.kind === 'parallel') {
    const path = pathOf(parentPath, definition.id);
    const settled = await Promise.allSettled(definition.children.map(child =>
      evaluate(child, runtime, resources, path, depth + 1).catch(error => {
        registerFailure(runtime, error);
        throw error;
      })));
...
export async function runProgram<Input, Output>(
  ...
  if (options.resources !== undefined && options.session !== undefined && options.resources !== options.session.resources) {
    throw new Error('A supplied session and resources must belong to the same run.');
  }
  const existingResources = options.resources ?? options.session?.resources;
  ...
  const decisions = options.session?.withResources(resources)
    ?? (options.provider === undefined ? undefined : new DecisionSession(options.provider, { resources }));
  ...
  try {
    ...
    resources.throwIfAborted();
    const validation = await validationDetail(output, options.validate, { signal: resources.signal, resources }, 'Final program validation failed.');
    ...
    if (error instanceof ProgramInvalidError || error instanceof ProgramDefinitionError) {
      await emit(runtime, { type: 'run-invalid', nodes, detail: error.message });
      outcome = { status: 'invalid', detail: error.message, metadata: metadataFor(runtime) };
    } else if (error instanceof LimitError && (error.evidence.kind === 'exhausted' || error.evidence.kind === 'deadline-exceeded')) {
      await emit(runtime, { type: 'run-exhausted', nodes, evidence: error.evidence });
      ...
      await emit(runtime, { type: 'run-cancelled', nodes, detail });
      outcome = { status: 'cancelled', detail, metadata: metadataFor(runtime) };
    } else {
      const detail = detailOf(error);
      await emit(runtime, { type: 'run-failed', nodes, detail });
